(function () {
    ...
    function openPeekModal(tab) {
        ...
        // Disable split when there's no current tab to split with, or when
        // the peeked tab IS the current tab.
        const canSplit = state.activeId !== null && state.activeId !== tab.id;
        if (!canSplit) {
    ...
    // Shows a small popover with tab metadata (title/number/account/contact/
    // primary resource) after a short hover-intent delay. Suppressed during
    // drag, with the context menu open, with the peek/settings/map modal open,
    // and on the Home tab (no tab metadata to show).
    ...
    function fillHoverCard(card, tab) {
        ...
        // Ticket-specific fields, only present when the iframe-bridge has
        // reported them (extracted from the ticket detail DOM).
    ...
    function positionHoverCard(card, anchorEl) {
        ...
        let top, left;
        if (isVerticalBar()) {
            // Vertical tab bar: place to the right of the tab.
            ...
            // Horizontal tab bar: place below the tab.
    ...
    function showHoverCardNow(tab, anchorEl) {
        ...
        // Pre-paint at off-screen position so we can measure size.
        card.style.left = '-9999px';
        card.style.top = '0px';
        card.classList.add('visible');
        // Next frame: real position with the now-known size.
        requestAnimationFrame(function () {
    ...
    function scheduleHoverCard(tab, anchorEl) {
        ...
        // If a card is already visible for any tab, swap quickly without delay.
